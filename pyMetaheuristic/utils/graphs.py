...
def plot_single_function(min_values, max_values, target_function, step = [0.1, 0.1], solution = [ ], proj_view = '3D', view = 'browser'):
    ...
    if (proj_view == '1D' or proj_view == '1d' or proj_view == '2D' or proj_view == '2d'):
        ...
        if (len(solution) > 0):
            sol   = solution[0]
            c_val = np.array([target_function(item) for item in sol])
            if ( len(min_values) > 2):
                if (sol.shape[0] == 1):
                    sol = np.vstack([sol, sol])
                sol, _, _ = svds(sol, k = 2)
...
def my_plot_single_function(min_values, max_values, target_function, step = [0.1, 0.1], solution = [ ], proj_view = '3D', view = 'browser'):
    if (view == 'browser' ):
        pio.renderers.default = 'browser'
    x    = []
    data = []
...
def full_plot(min_values, max_values, target_function, step = [0.1, 0.1], steps = [ ], solution = [ ], proj_view = '3D', view = 'browser'):
    if (view == 'browser' ):
        pio.renderers.default = 'browser'
    x    = []
    data = []
    ...
    if (proj_view == '1D' or proj_view == '1d' or proj_view == '2D' or proj_view == '2d'):
        ...
        if (len(solution) > 0):
            ...
            data.append(s_trace)
            s_trace = go.Scatter3d(x         = steps[:, 0],
                                   y         = steps[:, 1],
                                   z         = steps[:, -1],
                                   opacity   = 1,
                                   mode      = 'markers+text',
